...
<h3><span class="glyphicon glyphicon-briefcase"></span> EDUCATION </h3>
...
<?php 
$per_hal=10;
$jumlah_record=mysqli_query($koneksi,"SELECT COUNT(*) from education");
...
<table class="table table-hover">
	<tr>
		...
		<th class="col-md-1"><center>Kode education</center></th>
	...
	if(isset($_GET['cari_pnrbt'])){
		...
		$brg=mysqli_query($koneksi,"select * from education where nama like '%$cari%'");
	}else{
		$brg=mysqli_query($koneksi,"select * from education limit $start, $per_hal");
	}
	$no=1;
	while($b=mysqli_fetch_array($brg)){
		...
		<tr>
			...
			<td><center><?php echo $b['education_id'] ?></center></td>
			...
			<td>
			<center>
				<a href="edit_penerbit.php?education_id=<?php echo $b['education_id']; ?>" class="btn btn-warning">Edit</a>
				<a onclick="if(confirm('Apakah anda yakin ingin menghapus data ini ??')){ 
				   location.href='hapus_penerbit.php?education_id=<?php echo $b['education_id']; ?>' }" class="btn btn-danger">Hapus</a>
			</center>
			</td>
		</tr>		
		<?php 
...
<ul class="pagination">			
		...
		<?php
		$query = "SELECT max(education_id) as maxKode FROM education";
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form action="tmb_pnrbt_act.php" method="post">
					<div class="form-group">
						<label>Kode Penerbit</label>
						<input name="education_id" type="text"  readonly="" class="form-control" value="<?php echo $newID;?>">
					</div>
					<div class="form-group">
